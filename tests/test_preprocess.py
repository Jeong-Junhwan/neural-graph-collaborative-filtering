...
def test_load_train_data():
    train_data, data_info, laplacian_matrix = load_train_data()

    assert train_data["user_id"].max() == data_info["users"] - 1
    assert train_data["positive"].max() == data_info["users"] + data_info["items"] - 1
    assert len(train_data) == data_info["ratings"]
    assert laplacian_matrix.shape == (
